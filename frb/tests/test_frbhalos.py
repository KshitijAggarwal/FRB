...
def test_frac_in_halos():
    ...
    # Test
    np.testing.assert_allclose(ratios, np.array([0.46258579, 0.44314834, 0.36691678, 0.28180281]))
...
def test_halo_incidence():
    ...
    assert np.isclose(Navg, 22.13174071034526)
    # Cumulative now
    zeval, Ncumul = halos.halo_incidence(1e13, 1., Mhigh=3e13, cumul=True)
    assert zeval.size == 20
    assert np.isclose(Ncumul[-1], 11.153366432166912)

def test_YF17():
    yf17 = halos.YF17()
    ne = yf17.ne((0.,0.,20.))
    assert np.isclose(ne, 0.0008809671023270728)
